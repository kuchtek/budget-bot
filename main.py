...
def get_budget_from_airtable(category, month):
    ...
    params = {
        "filterByFormula": f"AND(Category='{category}', Month='{month}')"
    }
...
def get_categories_from_notion():
    ...
    if response.status_code == 200:
        results = response.json().get("results", [])
        categories = [entry["properties"]["Kategoria"]["title"][0]["text"]["content"] for entry in results]
...
async def add_category(update: Update, context: CallbackContext) -> None:
    try:
        ...
        if len(text) < 2:
            await update.message.reply_text('Błędny format. Użyj: /addcategory KATEGORIA')
            return
        ...
        if status_code != 200:
            await update.message.reply_text(f'Wystąpił błąd podczas dodawania kategorii do Notion: {response}')
            return
...
def update_budget_in_notion(category, month, amount):
    ...
    if response.status_code == 200:
        ...
        if results:
            ...
            data = {
                "properties": {
                    "Pozostało": {"number": new_remaining}
                }
            }
...
async def set_budget(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton(category, callback_data=f"setbudget_{category}") for category in categories]
    ]
...
async def handle_expense_input(update: Update, context: CallbackContext) -> None:
    try:
        ...
        if len(text) < 4:
            await update.message.reply_text('Błędny format. Użyj: WYDATEK KWOTA KATEGORIA OPIS')
            return
        ...
        if status_code != 200:
            await update.message.reply_text(f'Wystąpił błąd podczas dodawania wydatku do Notion: {response}')
            return

        await update.message.reply_text(f'Dodano wydatek: {amount} {category} - {description}')
    except Exception as e:
        await update.message.reply_text(f'Wystąpił błąd: {e}')
...
async def handle_budget_input(update: Update, context: CallbackContext) -> None:
    try:
        if 'selected_category' not in context.user_data:
            await update.message.reply_text('Najpierw wybierz kategorię używając /setbudget.')
            return
        ...
        if status_code != 200:
            await update.message.reply_text(f'Wystąpił błąd podczas dodawania budżetu do Notion: {response}')
            return

        await update.message.reply_text(f'Ustawiono budżet dla kategorii {category} na miesiąc {month} w wysokości {budget}')
...
async def handle_budget_confirmation(update: Update, context: CallbackContext) -> None:
    ...
    if choice == 'yes':
        ...
        await query.message.reply_text(f'Nadpisano istniejący budżet dla miesiąca {budget_month} w wysokości {existing_budget}.')
    elif choice == 'no':
...
async def add_expense(update: Update, context: CallbackContext) -> None:
    try:
        ...
        if len(text) < 5:
            await update.message.reply_text('Błędny format. Użyj: /add KATEGORIA KONTO WYDATEK OPIS')
            return
    
        category, account, amount, description = text[1], text[2], float(text[3]), text[4]
        current_date = datetime.now().strftime("%Y-%m-%d")
        month = datetime.now().strftime("%Y-%m")  # Extracting YYYY-MM

        # Dodaj wydatek do Notion
        status_code, response = add_expense_to_notion(current_date, category, account, amount, description )
        if status_code != 200:
            await update.message.reply_text(f'Wystąpił błąd podczas dodawania wydatku do Notion: {response}')
            return
        ...
        if status_code != 200:
            await update.message.reply_text(f'Wystąpił błąd podczas aktualizacji budżetu w Notion: {response}')
            return

        await update.message.reply_text(f'Dodano wydatek: {category} {account} {amount} {description}. Pozostało: {response["properties"]["Pozostało"]["number"]} PLN.')
    except Exception as e:
        await update.message.reply_text(f'Wystąpił błąd: {e}')
...
# # Stwórz application i przekaz mu token API bota
    ...
    application.add_handler(CallbackQueryHandler(set_budget_callback, pattern='^setbudget_'))
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_budget_input))
